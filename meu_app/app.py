...
@app.post('/comentario', tags=[comentario_tag],
          responses={"200": ComentarioSchema, "404": ErrorSchema})
def add_comentario(form: ComentarioSchema):
    """Adiciona um novo Comentario a um Produto cadastrado na base
    ...
    logger.debug(f"Adicionando comentário ao produto de id: '{comentario.produto_id}'")
    try:
        # criando conexão com a base
        session = Session()
        # buscando o produto associado ao comentário
        produto = session.query(Produto).filter(Produto.id == comentario.produto_id).first()
        if not produto:
            # se o produto não foi encontrado
            error_msg = "Produto não encontrado na base :/"
            logger.warning(f"Erro ao adicionar comentário ao produto de id '{comentario.produto_id}', {error_msg}")
            return {"mesage": error_msg}, 404
        ...
        logger.debug(f"Comentário adicionado ao produto de id: '{comentario.produto_id}'")
        # retorna a representação do comentário
        return apresenta_comentario(comentario), 200

    except Exception as e:
        # caso um erro fora do previsto
        error_msg = "Não foi possível adicionar novo comentário :/"
        logger.warning(f"Erro ao adicionar comentário ao produto de id '{comentario.produto_id}', {error_msg}")
        return {"mesage": error_msg}, 400